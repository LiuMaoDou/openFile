...
在左下角选择 **浅色 / 暗色 / 系统**。默认跟随系统，并随系统外观变化即时切换；手动选择会保存在本机，重启后继续使用。Windows 与 macOS 使用相同的主题，覆盖筛选、列表、文件详情、内容索引和操作弹窗。
...
扫描统一使用高性能模式，按本机 CPU 配置扫描和正文解析线程数，无需单独设置。旧版本保存的性能选项不再影响扫描。

扫描未完成时，展开扫描状态的 **详情**，即可查看各监控文件夹下未完成的具体路径、文件或文件夹类型，以及失败原因（包含系统返回的错误）。路径按监控文件夹去重并保存在本地，每页显示 50 项，可翻页或复制本页路径。核对成功后清除对应问题；子目录的核对不会清除其他目录的问题。旧版本只有失败数量的记录需要先点击 **刷新**，才能补充路径详情。

扫描时显示进度条、已处理/已发现目录数、当前位置和耗时。完整扫描先统计本批次目录，百分比表示**本批次已处理目录 / 已统计目录**；扫描中新发现的目录进入后续批次，不改变当前批次的分母。目录处理完成后单独显示“正在整理索引”，提交完成后才显示“扫描完成”。进度独立于搜索结果更新，不因刷新计时而重复检索。

扫描按批次合并多个目录的写入，未变化的文件不重复更新元数据索引。搜索与状态统计通过独立只读连接读取已提交的索引，扫描写入期间仍可检索；未变化的统计结果按索引版本复用。

普通文件新增、修改、删除与改名合并为按路径更新，不再每次重扫整个监控文件夹。目录变化优先核对受影响子树；监听溢出或需要重新核对的事件仍触发范围扫描；权限失败的旧索引与说明会保留。启动和手动刷新仍会完整核对，不能只根据父目录修改时间跳过深层文件。
...
点击一个文件，在 **文件详情** 中点击 **隐藏所在目录**，弹窗会自动填入该文件所在目录。也可以改选监控文件夹，或通过桌面版的文件夹图标选择其他目录。点击 **隐藏目录内容** 后，该目录及所有子目录中的文件会从普通列表和搜索结果中隐藏，不受当前搜索、类型或分页筛选限制；以后新增的文件也会遵循此规则。规则在刷新和重启后保留。

在 **已隐藏 → 管理目录规则** 中，或点击随目录隐藏的文件后，在详情中点击 **管理目录隐藏**，再点击对应目录的 **恢复显示** 即可移除规则。单独隐藏的文件和其他目录规则继续生效；随目录隐藏的文件需要先恢复对应目录。目录已离线或删除时，也可以移除规则。规则按目录路径保存，目录改名或迁移后需要重新选择新路径。

隐藏仅改变 FileM 的视图，文件仍保留在原位置并继续建立索引。数据库自动升级为版本 5，以保存目录规则，旧版应用不能打开升级后的索引。

## 批量移动

勾选多个文件 → **移动到…** → 选择目标文件夹 → 核对清单 → **确认移动**。已有同名文件、同一批次的重名项、身份变化和原地移动会跳过，不覆盖目标文件。在本次操作窗口查看逐项结果，支持停止后续移动和后台处理。
...
执行中可以停止后续文件或转到后台，在本次操作窗口查看逐项结果。停止不会撤销已经完成的文件。成功项从索引和选择中移除，失败项保留；中断或无法确认的项目显示“需要核对”，不会自动重试。
...
- 已实现普通文件事件按路径更新和目录变化按子树核对；尚未实现 USN/MFT 加速。
- 使用路径型遍历、并行扫描与独立只读查询连接；巨大单目录、百万文件、多文件夹同时写入仍需压力验证。目录隐藏规则按路径保存，不会自动跟随目录改名。
...
后续继续完善 Windows M0 与回收站实机验证、大规模扫描与异常恢复，再扩展复制和重命名。
